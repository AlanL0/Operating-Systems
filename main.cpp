...
void edf_schedule(void)
{
	//LCM = 12000
	/*
	 * for each ready process executes the deadline closer to current time
	 * */
	long deadline = 120000;
	int thread;
	int run_id = 0;
	int current=0;
	ThreadCtrlBlk temp;
	...
	if(occupied ==0 && readyQue.size()>0)
	{
		for(std::vector<int>::iterator ready = readyQue.begin(); ready!= readyQue.end(); ++ready)
			{
				//look for closest deadline
				thread = *(ready);
				temp = tcb[thread];
				...
				if(temp.deadline < deadline)
				{
					...
					run_id = current;
					//printf("Run ID is : %d and size is %d\n", run_id, readyQue.size());
					//printf(" DEADLINE IS FOR %d at %d\n", temp.id, deadline);
				}
		...
		pthread_mutex_lock(&mutex);
		pthread_cond_signal(&cond[readyQue[run_id]]); // Wake up the earliest deadline
		readyQue.erase(readyQue.begin() + run_id);    // erase from ready queue
		pthread_mutex_unlock(&mutex);
...
void rm_schedule(void)
{
	...
	if(occupied ==0 && readyQue.size()>0)
	{
		for(std::vector<int>::iterator ready = readyQue.begin(); ready!= readyQue.end(); ++ready)
			...
			pthread_mutex_lock(&mutex);
			pthread_cond_signal(&cond[readyQue[run_id]]); // Wake up the closest period
			readyQue.erase(readyQue.begin() + run_id);    // erase from ready queue
			pthread_mutex_unlock(&mutex);
